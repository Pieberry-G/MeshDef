...
bool MeshTools::CreateMesh(Mesh & mesh, const std::vector<std::array<double, 3>>& vertices, const std::vector<std::array<int, 3>>& faces)
{
	mesh.clear();
	...
	{
		Mesh::Point point(vertices[i][0], vertices[i][1], vertices[i][2]);
		vertexHandles.push_back(mesh.add_vertex(point));
	}
	...
	for (int i = 0; i < faces.size(); ++i)
	{
		...
		for (int j = 0; j < 3; ++j)
		{
			faceVertices.push_back(vertexHandles[faces[i][j]]);
		}
...
bool MeshTools::WriteMesh(const Mesh & mesh, const std::string & filename, const std::streamsize & precision)
{
	...
	if (ext == "obj")
	{
		if (mesh.has_halfedge_texcoords2D())
			return WriteFaceTextutedOBJ(mesh, path + "/" + basename + "." + ext, precision);
		if (mesh.has_vertex_texcoords2D())
			return WriteVertexTextutedOBJ(mesh, path + "/" + basename + "." + ext, precision);
...
bool MeshTools::WriteVertexTextutedOBJ(const Mesh& mesh, const std::string& filename, const std::streamsize& precision)
{
	if (!mesh.has_vertex_texcoords2D()) return false;
	std::ofstream ofs(filename);
	if (!ofs.is_open())
	{
		std::cerr << "Error: cannot open file " << filename << std::endl;
	...
	size_t sp = filename.find_last_of('/') + 1;
	std::string filepath = filename.substr(0, sp);
	std::string basename = filename.substr(sp);
	ofs << "mtllib " + basename + ".mtl\n\n";
	ofs.precision(precision);
	for (const auto& vh : mesh.vertices())
	{
		ofs << "v " << mesh.point(vh) << std::endl;
		ofs << "vt " << mesh.texcoord2D(vh) << std::endl;
	}
	for (const auto& fh : mesh.faces())
	{
		ofs << "f";
		for (const auto& fvh : mesh.fv_range(fh))
		{
			ofs << " " << fvh.idx() + 1 << "/" << fvh.idx() + 1;
		}
		ofs << std::endl;
	}
	ofs.close();

	ofs.open(filename + ".mtl");
	if (!ofs.is_open())
	{
		std::cerr << "Error: cannot open file " << filename + ".mtl" << std::endl;
		return false;
	}
...
bool MeshTools::WriteFaceTextutedOBJ(const Mesh& mesh, const std::string& filename, const std::streamsize& precision)
{
	if (!mesh.has_halfedge_texcoords2D()) return false;
	std::ofstream ofs(filename);
	if (!ofs.is_open())
	{
		std::cerr << "Error: cannot open file " << filename << std::endl;
	...
	{
		ofs << "vt " << mesh.texcoord2D(hh) << std::endl;
	}
	for (const auto& fh : mesh.faces())
	{
		ofs << "f";
		for (const auto& fhh : mesh.fh_range(fh))
		{
			ofs << " " << mesh.to_vertex_handle(fhh).idx() + 1 << "/" << fhh.idx() + 1;
		}
		ofs << std::endl;
	}
	ofs.close();
